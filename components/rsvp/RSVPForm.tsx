  ...
  return (
    <section id="rsvp" className="relative overflow-hidden py-28 md:py-36">
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 50% 50%, rgba(139,30,63,0.35), transparent 60%), linear-gradient(180deg, #0a0807 0%, #1A0409 100%)",
        }}
      />
            ...
            {!submitted ? (
              ...
              >
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  >
                    <input
                      ...
                      placeholder="Your graceful name"
                      className="input"
                    />
                  </Field>
                ...
                <div className="mt-6 grid gap-6 md:grid-cols-2">
                  <SelectGroup
                    label="Attending Sangeet?"
                    value={form.attendingSangeet}
                    onChange={(v) =>
                      setForm({ ...form, attendingSangeet: v as "yes" | "no" | "maybe" })
                    }
                    options={[
                  ...
                  />
                  <SelectGroup
                    label="Attending Barat?"
                    value={form.attendingBarat}
                    onChange={(v) =>
                      setForm({ ...form, attendingBarat: v as "yes" | "no" | "maybe" })
                    }
                    options={[
                      { value: "yes", label: "Joyfully Yes" },
                      { value: "maybe", label: "Maybe" },
                      ...
                      {(["veg", "non-veg", "jain"] as const).map((opt) => (
                        <button
                          ...
                          className={`rounded-full border px-5 py-2 font-serif text-sm capitalize tracking-wide transition ${
                            form.food === opt
                              ? "border-gold-foil bg-gold-foil/15 text-gold-shimmer"
...
function SelectGroup({
  ...
  return (
    <div>
      <span className="mb-2 block font-serif text-xs uppercase tracking-[0.3em] text-champagne-200/70">
        {label}
      </span>
  ...
  return (
    ...
    >
      ...
      {Array.from({ length: 12 }).map((_, i) => (
        <motion.span
          key={`p-${i}`}
          className="absolute left-1/2 top-1/2 h-3 w-2 rounded-full"
          style={{
            background: ["#C89B7B", "#E8C4A8", "#F4D8C3"][i % 3],
          }}
      ...
      <p className="relative mx-auto mt-5 max-w-xl font-serif text-lg italic text-ivory/80">
        Your response has been safely received. Our hearts are already dancing
        in anticipation. See you under the marigolds.
      </p>
      <div className="relative mx-auto mt-6 max-w-xs">
        <OrnateDivider glyph="star" />
      </div>
      <p className="relative mt-6 font-script text-3xl text-rose-gold">
        — Anukriti &amp; Anmol
      </p>
